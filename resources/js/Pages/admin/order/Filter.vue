...
<template>
    ...
    <VFilter align="right" @apply="applyFilter" @clear="clearFilter">
        <div class="grid grid-cols-2 gap-3 px-4 pb-4 pt-1.5">
            <div>
                <div class="text-xs font-semibold text-slate-400 uppercase mb-1">Filter by Customer</div>
                <VSelect placeholder="Select" v-model="filter.filter_customer"
                    :options="additional.customer_list" :clearable="false" ref="filterCustomer" />
            </div>
        </div>
    </VFilter>
</template>