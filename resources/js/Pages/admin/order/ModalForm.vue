...
const submit = async () => {
   ...
   axios.put(route('order.updateqtycart', { 'id': props.data.id }), form.value)
       .then((res) => {
           emit('close')
           emit('successUpdate')
           form.value = ref({})
       ...
       <template v-slot:content>
           <div class="grid grid-cols-2 gap-3">
               <div class="col-span-2">
                   <VInput placeholder="Insert Quantity" label="Quantity" :required="true" v-model="form.quantity" :errorMessage="formError.quantity"
                       @update:modelValue="formError.quantity = ''" />
               </div>
           </div>
       </template>
